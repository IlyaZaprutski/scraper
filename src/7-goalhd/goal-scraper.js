...
const download = (url, dest) => new Promise((resolve, reject) => {
    fs.promises.mkdir('./tmp/7-goalhd', { recursive: true });
...
const scrapeHighlights = async (count) => {
    const highlightUrls = await getHighlightUrls();

    const highlights = await Promise.all(highlightUrls.slice(0, count).map(url => parseHighlight(url)));
